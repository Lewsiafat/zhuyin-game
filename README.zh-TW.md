...
- 打出注音符號對怪物造成傷害
- 打錯：扣 2 秒時間 + 正確目標鍵黃色高亮提示（0.8 秒）
- 時間到：扣 1 顆 HP（共 3 顆）
- 每完成 3 句：升級（更難的句子、更強的怪物）並顯示升級動畫
- 每第 5 關：Boss 關卡（HP ×3、體型 ×1.5、金橘色光暈）
- 連擊系統：連續答對每 5 下觸發爆擊（×2 傷害）
- ESC 或 HUD ⏸ 按鈕可隨時暫停；暫停選單含音效/音樂開關、新遊戲、停止遊戲
- 最高分儲存於 localStorage
- 結束畫面顯示正確率（%）與最高連擊數
- 支援實體鍵盤與螢幕虛擬鍵盤
- 分層句庫：100 句分三階（初學／中級／高級）
- 背景音樂（BGM）：Web Audio API 程序生成旋律，可在暫停選單切換
- 成就系統：8 個條件型成就，Toast 動畫通知 + localStorage 持久化
- 每日挑戰：以當天日期為 seed 選出固定 5 句題組，每日限玩一次
- 教學關卡：10 步驟互動引導 + 鍵盤高亮，完成後解鎖成就
